...
public class QueueRunner implements CommandLineRunner {
    ...
    public QueueRunner(RabbitTemplate rabbitTemplate, UserQueueMapper userQueueMapper) {
    ...
    public void createQueue() throws IOException, TimeoutException {
        ...
        userQueueDOS.forEach(queue -> {
            try {
                ...
                log.error("-->创建队列:" + queue + "异常! 详细内容：" + exception);
                throw new ApplicationException(String.format("创建队列: %s 异常!", queue), EnumExceptionCode.InternalServerError);
            }
        });
